...
namespace AIO.IDOS3.Mobile.Data.Entity
{
    ...
    public partial class mvProductLotSyncProvider : DataSyncProvider<mvProductLot>
    {
        ...
        public static string Selector = "ID,Code,ProductID,ExpiredDate,ProductLot,StatusID,StatusName,SAPCode,CreatedDate,ModifiedDate,IsDeleted";
        ...
        protected override Exception OnDownloadData(mvProductLot obj, bool useTransaction) { return OnDownloadDataAsync(obj, useTransaction).Result; }
        protected override Exception OnUploadData(mvProductLot obj, bool useTransaction) { return OnUploadDataAsync(obj, useTransaction).Result; }
        ...
        protected override async Task<Exception> OnDownloadAllDataAsync(bool continueOnError)
        {
            ...
            var productLotTableProvider = DbContext.GetDataTableProvider<mvProductLotTableProvider>();
            var productLotServiceProvider = DataServiceContext.GetDataServiceProvider<mvProductLotServiceProvider>();

            DateTime? syncDate = null;

            List<mvProductLot> serverDataList = null;

            var minSyncDate = (DateTimeOffset?)await productLotTableProvider.GetData().MinAsync(p => p.SyncDate);
            ...
            for (int i = 0; i < DbContext.SyncMaxAttempts; i++)
            {
                try
                {
                    ...
                    var query = (IQueryable<mvProductLot>)productLotServiceProvider.GetData().AddQueryOption("$select", Selector);
            ...
            if (serverDataList != null)
            {
                if (serverDataList.Count > 0)
                {
                    ...
                    foreach (var serverData in serverDataList)
                    {
                        try
                        {
                            ...
                            await ProcessLocalData(syncDate, 1, 1, serverData, null, productLotTableProvider, true);
                        }
                        catch (Exception ex)
                        {
                            if (!continueOnError)
                ...
                await DbContext.UpdateAllSyncDateAsync<mvProductLot>(syncDate.Value);
            }
        ...
        protected override async Task<Exception> OnDownloadDataAsync(mvProductLot obj, bool useTransaction)
        {
            Exception exception = null;
            ...
            for (int i = 0; i < DbContext.SyncMaxAttempts; i++)
            {
                try
                {
                    ...
                    serverData = productLotServiceProvider.GetData().AddQueryOption("$select", Selector)
            ...
            await ProcessLocalData(syncDate, status, attempts, serverData, obj, productLotTableProvider, useTransaction);

            return exception;
        }

        protected override async Task<Exception> OnUploadDataAsync(mvProductLot obj, bool useTransaction)
        {
            Exception exception = new NotImplementedException();
        ...
        private async Task ProcessLocalData(DateTime? syncDate, int status, int attempts, mvProductLot serverData, mvProductLot obj,
            mvProductLotTableProvider tableProvider, bool useTransaction)
        {
            var localDate = (syncDate.HasValue) ? syncDate.Value : await DbContext.GetDatabaseUtcDateTimeAsync();
            mvProductLot productLot = null;

            if (serverData == null)
            {
                productLot = new mvProductLot();
                productLot.ID = obj.ID;

                await tableProvider.DeleteDataAsync(productLot, useTransaction);
            }
            else
            {
                var isInsert = false;
                var isDelete = false;
                productLot = await tableProvider.GetDataAsync(serverData.ID);
                if (productLot == null)
                {
                    isInsert = true;
                    productLot = new mvProductLot();
                }
                else if (productLot.IsDeleted)
                    isDelete = true;

                productLot.CopyFrom(serverData);
                DbContext.SetSyncData(productLot, syncDate, localDate, status, attempts);

                if (isInsert)
                    await tableProvider.InsertDataAsync(productLot, useTransaction);
                else if (isDelete)
                    await tableProvider.DeleteDataAsync(productLot, useTransaction);
                else
                    await tableProvider.UpdateDataAsync(productLot, useTransaction);

                if (obj != null)
                {
                    obj.CopyFrom(serverData);
                    DbContext.CopySyncData(obj, productLot);
                }
            }
        }

        private void AssignFromLocalData(mvProductLot serverData, mvProduct product)
        {
            if (product != null)
            {
