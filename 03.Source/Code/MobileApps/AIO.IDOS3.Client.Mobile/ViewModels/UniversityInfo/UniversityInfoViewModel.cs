...
{
    public class UniversityInfoViewModel : ViewModelBase<ApplyUniversitiesModel>
    {

        #region Constructors
        public UniversityInfoViewModel(INavigationService navigationService, Prism.Services.IPageDialogService pageDialogService, IDialogService dialogService)
            : base(navigationService, pageDialogService, dialogService)
        {
            ...
            NavigateToDetailsCommand = new Command(async () => await OnNavigateToDetails());
        ...
        protected async Task OnNavigateToDetails()
        {
            if (!IsBusy)
            {
                IsBusy = true;
