...
namespace Wismapi.Data.Entity
{
    ...
    public abstract class CoreDataServiceContext : DataServiceContext
    {
        ...
        {
            return DateTime.Parse(await CreateFunctionQuerySingle<string>("", "GetDatabaseServerDateTime", true).GetValueAsync());
        }

        public async Task<DateTime> GetDatabaseServerUtcDateTimeAsync()
        {
            return DateTime.Parse(await CreateFunctionQuerySingle<string>("", "GetDatabaseServerUtcDateTime", true).GetValueAsync()).ToUniversalTime();
        }
        ...
        private void InternalSaveDeepChanges<TData>(BaseEntityType data, EntityStates state) where TData : BaseEntityType
        {
            ...
            {
                ContractResolver = DataDefaultContractResolver.Instance,
            ...
            using (var httpHandler = new HttpClientHandler())
            {
                using (var httpClient = new HttpClient(httpHandler))
                {
                    ...
                    using (var odataContent = new StringContent(serializedEntity))
                    {
                        ...
                        var responseData = JsonConvert.DeserializeObject<TData>(content);
